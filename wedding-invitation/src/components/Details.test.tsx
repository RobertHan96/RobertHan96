...
describe('wedding details', () => {
  ...
  it('shows account details without an RSVP section', () => {
    ...
    expect(screen.getByText('신한은행 110-467-266513')).toBeInTheDocument()
    expect(screen.getByText('신랑 한영신', { selector: '.account-meta' })).toBeInTheDocument()
    expect(screen.getByText('우리은행 1002-353-385470')).toBeInTheDocument()
    expect(screen.getByText('신부 이다예', { selector: '.account-meta' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '신부 이다예 계좌번호 복사' })).toBeVisible()
    expect(screen.getByText('우리은행 1002-230-989004')).toBeInTheDocument()
    expect(screen.getByText('신부 어머니 오영근', { selector: '.account-meta' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '신부 어머니 오영근 계좌번호 복사' })).toBeVisible()
    expect(screen.getByText('신한은행 110-360-443299')).toBeInTheDocument()
    expect(screen.getByText('신부 아버지 이성환', { selector: '.account-meta' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '신부 아버지 이성환 계좌번호 복사' })).toBeVisible()
